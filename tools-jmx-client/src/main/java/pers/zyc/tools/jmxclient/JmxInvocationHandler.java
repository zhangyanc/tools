...
public class JmxInvocationHandler implements InvocationHandler {
	...
	static <T> T createMBeanProxy(MBeanServerConnection connection,
											Class<T> interfaceClass, ObjectName objectName) {
		if (!interfaceClass.isInterface()) {
			throw new JmxException(interfaceClass.getName() + " is not interface");
		}
		return interfaceClass.cast(Proxy.newProxyInstance(interfaceClass.getClassLoader(),
				new Class<?>[] {interfaceClass}, new JmxInvocationHandler(connection, interfaceClass, objectName)));
	}
	...
	private JmxInvocationHandler(MBeanServerConnection connection, Class<?> interfaceClass, ObjectName objectName) {
